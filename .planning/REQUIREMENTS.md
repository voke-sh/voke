...
- [ ] **SPEC-04**: A machine-readable rule registry (one entry per rule) is the single source of truth shared by spec and linter; a build check fails on any rule-in-code without a registry entry
- [ ] **SPEC-05**: SCOPE.md documents the L1 boundary (no LLM-in-loop; no gateway/proxy) to prevent scope creep

### Engine

- [ ] **ENG-01**: A Spectral-shaped rule engine runs rules typed as pure synchronous functions `(context) => Finding[]` (no IO, no model)
- [ ] **ENG-02**: Engine supports both per-tool rules and server-aggregate (surface-level) rules
- [ ] **ENG-03**: Rule registry is a startup-time plugin boundary (fresh sealed instance; custom/vendor rules register before seal); `voke.yaml` severity overrides produce a new registry without mutating the default
...
- [ ] **RULE-01**: Schema-correctness rules (S01–S08) implemented with positive + negative fixtures
...
- [ ] **PUB-01**: MTQS spec is published and versioned at voke.sh/spec in a public repo accepting PRs
...
|-------------|-------|--------|
| SPEC-01..05 | TBD | Pending |
| ENG-01..04 | TBD | Pending |
| ING-01..05 | TBD | Pending |
| RULE-01..06 | TBD | Pending |
| SCORE-01..02 | TBD | Pending |
| OUT-01..02 | TBD | Pending |
| CLI-01..03 | TBD | Pending |
| CI-01..02 | TBD | Pending |
| PUB-01..04 | TBD | Pending |
